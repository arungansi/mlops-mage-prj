...
def create_visualization(inputs: Tuple[Booster, DataFrame, Series], *args, **kwargs):
    model, X, _ = inputs
    X_np = X.to_numpy()

    # Random sampling - for example, 10% of the data
    sample_indices = np.random.choice(X_np.shape[0], size=int(X_np.shape[0] * 0.1), replace=False)
    X_sampled = X_np[sample_indices]
    X_sampled = X_np[:1]
